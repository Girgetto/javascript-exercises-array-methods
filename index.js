...
function getAverage(numbers) {
  return numbers.reduce((i, number) => i+(number/numbers.length), 0);
}
...
function getRepetitions(array) {
  return array.reduce((object, string) => {
     if (!object[string]) {
        object[string] = 1;
     } else {
...
function getSumAges(people) {
  return people.reduce((i, user) => user.age + i, 0);
}
